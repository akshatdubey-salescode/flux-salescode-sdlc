...
export const COMPLEXITY_LOC_NOTE = {
  ...
  steps: [
    ...
    "The four Jira Complexity Rating columns form a 2x2 grid: {all-Jiras, non-self-assigned (NSA)} x {marked complexity, LOC-predicted (\"expected\") complexity}. Complex. (Mar) and Complex. (Exp) keep the same all-Jiras population as Score — Complex. (Mar) is in fact the identical formula and value as Score, just labeled for the grid. Complex. NSA. (Mar) and Complex. NSA. (Exp) exclude self-assigned Jiras (reporter === credited person) entirely, at attribution time, from every metric. None of the four ever touch Score's own drill-down lists (Weighted bugs, Feature tasks, MTTR samples, etc.) — those stay Score's, unfiltered.",
    "Within each population, Marked vs. Expected differ only in the Complex Tasks metric: Marked uses each task's marked complexity (the actual rating), Expected uses the LOC-predicted complexity instead. Bug Quality, Sprint Commitment, and MTTR are complexity-agnostic, so they're identical between Marked and Expected within the same population.",
    "Complexity Accuracy is correct/checked tasks, shown as a % — how many had marked complexity match what the LOC predicts. Read in the Details drill-down (not the leaderboard), with two separate readings: one over every task (all-Jiras), one restricted to non-self-assigned tasks. Nothing is excluded for having an unmarked complexity or no matched PR — both default to C1 rather than being dropped from the count.",
