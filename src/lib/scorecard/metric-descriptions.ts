...
// Self-assigned exclusion + the two Score columns + "Complexity Acc." + the
// C4/C5-vs-LOC flag. All LOC-based, all baked into both scores.
export const COMPLEXITY_LOC_NOTE = {
  title: "Self-assigned exclusion, the two scores, complexity accuracy & flags",
  intro: "Every score already reflects these — not optional second opinions:",
  steps: [
    "Self-assigned Jiras — issues where the reporter is also the person credited for the work — are excluded entirely, at attribution time, from every metric, in both scores. They never reach either rating, or any drill-down list.",
    "Two ratings, same formula: Score (Marked) weights the Complex Tasks metric by each task's marked complexity — the actual rating. Score (Expected) uses the identical formula but weights it by the LOC-predicted complexity instead. Bug Quality, Sprint Commitment, and MTTR are complexity-agnostic, so they're always the same between the two — only the Complex Tasks contribution (and therefore the total) can differ.",
