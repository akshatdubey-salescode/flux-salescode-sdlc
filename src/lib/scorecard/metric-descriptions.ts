...
export const COMPLEXITY_LOC_NOTE = {
  ...
  steps: [
    ...
    "The four Jira Complexity Rating columns form a 2x2 grid: {all-Jiras, non-self-assigned (NSA)} x {marked complexity, LOC-predicted (\"expected\") complexity}. Complex. (M) and Complex. (E) keep the same all-Jiras population as Score — Complex. (M) is in fact the identical formula and value as Score, just labeled for the grid. Complex. NSA. (M) and Complex. NSA. (E) exclude self-assigned Jiras (reporter === credited person) entirely, at attribution time, from every metric. None of the four ever touch Score's own drill-down lists (Weighted bugs, Feature tasks, MTTR samples, etc.) — those stay Score's, unfiltered.",
