...
// "LOC Sync Status" button/modal on the developer drill-down — mirrors
// MetricMeaningModal's placement/pattern (button next to the heading, Dialog
// on click). Fetches lazily on open (not on page load, since the reason
// diagnosis does real per-Jira queries) and lets the developer download the
// same rows as a single CSV, following Bug Board's fetch→blob→<a download>
// export pattern (bug-board-client.tsx's handleExportExcel).
...
export function LocSyncStatusModal({ email, quarterKey }: { email: string; quarterKey: string }) {
  ...
  // an outside click only closes this picker, never the main modal behind it.
    ...
    <>
      <Dialog open={open} onOpenChange={handleOpenChange}>
        ...
        >
          ...
          <div className="-mx-4 -mb-4 min-h-0 flex-1 space-y-3 overflow-y-auto px-4 pb-4">
            {loading && (
      ...
      {/* Independently-controlled nested modal for the "more than one PR"
          ...
          this picker via its own overlay — the LOC Sync Status modal
          underneath never closes with it. */}
