...
export function AtlassianIntegrationCard() {
  ...
  async function disconnect() {
    ...
    try {
      await fetch("/api/integrations/atlassian", { method: "DELETE" });
    ...
    <div className="rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          {/* Atlassian wordmark "A" */}
          <div className="flex size-10 items-center justify-center rounded-lg bg-[#0052CC] text-white font-bold text-lg shrink-0">
            A
          </div>
          <div>
            <p className="text-sm font-semibold text-foreground">Atlassian</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              Publish requirements directly to Jira as your own account.
            </p>
          </div>
        </div>
