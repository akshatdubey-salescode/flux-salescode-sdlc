    ...
    <main className="relative min-h-screen w-full flex flex-col items-center justify-center bg-zinc-50 dark:bg-zinc-950 px-4 py-12 overflow-hidden select-none">
      ...
      <div className="w-full max-w-[440px] z-10 transition-all duration-300">
        <div className="bg-white/70 dark:bg-zinc-900/40 backdrop-blur-xl border border-zinc-200/50 dark:border-zinc-800/40 shadow-2xl shadow-zinc-200/30 dark:shadow-black/70 rounded-3xl p-8 sm:p-10 space-y-8">
          ...
          {/* Logo with pulsing glowing circles */}
          ...
          {/* Elegant error alert if sign-in failed */}
          {error && (
          ...
          <div className="space-y-4">
            ...
            <p className="text-center text-xs text-zinc-400 dark:text-zinc-500 font-medium">
              Authorized domain restriction: @salescode.ai
            </p>
          </div>

          {/* Secure details tag */}
          <div className="pt-4 border-t border-zinc-200/50 dark:border-zinc-800/40 flex items-center justify-center gap-2 text-xs text-zinc-400 dark:text-zinc-500">
            <ShieldCheck className="h-4 w-4 text-emerald-500 dark:text-emerald-400 shrink-0" />
            <span>Secure, single sign-on powered by Google</span>
          </div>
      ...
      {/* Footer navigation */}
      <div className="absolute bottom-6 flex items-center gap-4 text-xs text-zinc-400 dark:text-zinc-500 z-10 font-medium">
        <a href="/" className="hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors">Flux Home</a>
        <span className="h-3 w-px bg-zinc-200 dark:bg-zinc-800" />
        <a href="/privacy" className="hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors">Privacy Policy</a>
      </div>
    </main>
  );
}
