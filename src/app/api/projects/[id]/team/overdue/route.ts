...
export type {
  ...
  OverduePersonGroup,
  OverdueResponse,
} from "@/app/api/observer/boards/[boardId]/overdue/route";

import type { OverdueIssueItem, OverduePersonGroup, OverdueResponse } from "@/app/api/observer/boards/[boardId]/overdue/route";
...
export async function GET(req: Request, { params }: Params) {
  try {
    ...
    const data = await fetchProjectOverdue(projectId, today, quarterStart, quarterEnd);
    if (data === null) return NextResponse.json({ error: "Not found" }, { status: 404 });
    return NextResponse.json(data);
  } catch (err) {
    console.error("[project-team-overdue] error:", err);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}

async function fetchProjectOverdue(projectId: string, today: string, quarterStart: string, quarterEnd: string) {
  "use cache";
  cacheLife("minutes");
  ...
  if (members.length === 0) return { today, quarterStart, quarterEnd, totalCount: 0, byPerson: [] } satisfies OverdueResponse;
  ...
  for (const raw of issuesRes.rows as IssueRow[]) {
    ...
    const daysOverdue = Math.ceil((new Date(today).getTime() - new Date(dueDate).getTime()) / 86400000);
  ...
  for (const issues of byEmail.values()) issues.sort((a, b) => b.daysOverdue - a.daysOverdue);
  ...
  for (const m of members) {
    ...
    byPerson.push({ email: m.email, name: m.name, isManager: false, issues, maxDaysOverdue: issues[0].daysOverdue });
  }
