...
export async function PATCH(
  ...
  if (body.escalationMultiplier !== undefined) {
    if (typeof body.escalationMultiplier !== "number" || body.escalationMultiplier <= 1) {
      return Response.json(
        { error: "escalationMultiplier must be greater than 1" },
        { status: 400 }
      );
    }
    patch.escalationMultiplier = String(body.escalationMultiplier);
  }
