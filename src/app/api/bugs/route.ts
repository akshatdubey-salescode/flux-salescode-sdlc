...
export type BugCell = {
  ...
  /** Normalized environment label (Prod / UAT / Demo / … / "—" when unset). */
  environment: string;
  total: number;
  p1: number;
  p2: number;
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Default to the current fiscal quarter so a bare hit returns the same
    // window the board shows on load.
...
async function fetchBugBoard(from?: string, to?: string): Promise<BugBoardResponse> {
  ...
  const toFilter = to
    ...
    : sql``;
  // Bug-family issue types (bug / defect / sub-bug), matching the rest of the app.
