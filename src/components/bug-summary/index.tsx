...
export function BugSummaryTab({ projectId }: Props) {
  ...
  const summaries = useMemo(() => buildOwnerSummaries(bugs), [bugs]);
  const hasOther = useMemo(() => bugs.some((b) => b.priorityBucket === "Other"), [bugs]);
  ...
  // Distinct environment labels present, in a stable preferred order.
  ...
  const filteredSorted = useMemo(() => {
    ...
    list = [...list].sort((a, b) => {
      ...
      // Stable secondary sort by key so equal rows don't jump around.
      if (cmp === 0) cmp = a.jiraKey.localeCompare(b.jiraKey, undefined, { numeric: true });
      return cmp * dir;
    });
    return list;
  }, [bugs, query, envFilter, ownerFilter, sortKey, sortDir]);
    ...
    return (
      <div className="rounded-lg border border-dashed border-zinc-200 p-12 text-center dark:border-zinc-800">
        ...
        <p className="mt-1 text-xs text-zinc-400">
          Bug-type issues will appear here once they’re synced from Jira.
        </p>
      </div>
    );
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-sm font-semibold text-zinc-800 dark:text-zinc-100">
            ...
            <span className="ml-2 text-xs font-normal text-zinc-400">
              {filteredSorted.length} of {bugs.length}
            </span>
          </h2>
