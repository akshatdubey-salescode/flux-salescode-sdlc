...
import type { GlobalSearchFilterState, GlobalSearchFields } from "./helpers";

type Props = {
  filters: GlobalSearchFilterState;
  fields: GlobalSearchFields | null;
  onUpdate: (updates: Partial<Record<string, string | null>>) => void;
  total: number;
};

export function SearchFilterBar({ filters, fields, onUpdate, total }: Props) {
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative min-w-[200px] flex-1 max-w-md">
          ...
          <Input
            ...
            placeholder="Search by key, summary, or across all issues…"
            className="pl-7"
          />
        </div>
...
function ActiveChips({
  ...
  filters.assignee.forEach((a) => {
    const assignee = fields?.assignees.find(as => as.email === a);
    chips.push({
      label: `Assignee: ${assignee?.name || a.split("@")[0]}`,
      onRemove: () =>
        onUpdate({
  ...
  filters.reporter.forEach((r) => {
    const reporter = fields?.reporters.find(rep => rep.email === r);
    chips.push({
      label: `Reporter: ${reporter?.name || r.split("@")[0]}`,
      onRemove: () =>
        onUpdate({
        ...
        <div className="flex-1 overflow-y-auto space-y-5 p-6">
            ...
            <FilterSection label="Assignee">
              {fields.assignees.map((a) => (
                <CheckRow
                  key={a.email}
                  label={a.name}
                  checked={filters.assignee.includes(a.email)}
                  onChange={() =>
                    toggleMulti("assignee", filters.assignee, a.email)
                  }
                />
              ))}
            </FilterSection>
          ...
          <FilterSection label="Other">
            <div className="flex items-center gap-2.5">
              ...
              />
              <Label htmlFor="gs-has-comments" className="cursor-pointer text-xs font-normal text-zinc-700 dark:text-zinc-300">
                Has comments
              </Label>
            </div>
          </FilterSection>
